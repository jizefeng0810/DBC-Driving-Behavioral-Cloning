...
PID_PARAMS = [
        ...
        dict(K_P=1.5,  K_I=2.5, K_D=0.05,  fps=20, n=80),   # add
        dict(K_P=2,  K_I=3.0, K_D=0.1,  fps=20, n=40),
        ]
...
class Controller(object):
    def __init__(self, pid):
        ...
        self.speed_control = PIDController(K_P=1, K_I=1.0, K_D=0.5, fps=20, n=100)
        # self.speed_control = PIDController(K_P=0.75, K_I=2.0, K_D=0.5, fps=20, n=50)
    ...
    def run_step(self, target, target_speed, speed, viz=None):
        # target_speed = np.clip(target_speed, 0.0, 4)    # 0 - 4
        target_speed = np.clip(target_speed, 0.0, 8)  # 0 - 20
        # target_speed = np.clip(target_speed, 0.0, 10)  # 0 - 20
