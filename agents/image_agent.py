...
class Visualizer(object):
    ...
    def controller_draw(self, speed, target_speed, control):
        ...
        self.draw.text((5, 20), 'Current Speed: %.2f m/s' % (speed))
        # self.draw.text((5, 30), 'Target Speed: %.2f m/s' % target_speed)
...
class ImageAgent(object):
    ...
    def run_step(self, observations):
        ...
        if self.step == 0:
            self.rgb_forward = rgb

        target, target_speed = self.planner.run_step(rgb, self.rgb_forward, viz)  # run net
