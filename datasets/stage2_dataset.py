...
class CarlaDataset(Dataset):
    def __init__(self, dataset_dir, transform=transforms.ToTensor(), target='map', **kwargs):
        ...
        for image_path in sorted(dataset_dir.glob('image*.png')):
            if image_path.stem == 'image_0000': continue
    ...
    def __getitem__(self, idx):
        ...
        num_str = '0' * (4 - len(str(idx))) + str(idx)
        target_forward = np.load(str(path / ('birdview_%s.npy' % num_str)))
        target_forward = crop_birdview(target_forward, dx, dy)
        target_forward = self.to_tensor(target_forward)  # (7,192,192)
        target_forward = target_forward[[0, 5]]  # (2,192,192)
...
if __name__ == '__main__':
    ...
    for i in range(len(data)):
        ...
        for idx, (x, y) in enumerate(waypoints_rgb):
            x = AUG_MAP_SIZE + (AUG_MAP_SIZE - x)
            draw_rgb.ellipse((x - 2, y - 2, x + 2, y + 2), fill=(0, 0, 255))
            x = x + (idx + 1.) / 2.5
            draw_rgb.ellipse((x - 2, y - 2, x + 2, y + 2), fill=(0, 255, 0))
            x = x + (idx+1.)/2
            # x, y = x + (idx + 1) / 1.5 + 2, y+1
            draw_rgb.ellipse((x - 2, y - 2, x + 2, y + 2), fill=(255, 0, 0))

        if i >= 340 and i < 341:
            print('save')
            # cv2.imwrite('mappoint_%d.jpg' % i, cv2.cvtColor(np.array(canvas), cv2.COLOR_BGR2RGB))
            cv2.imwrite('../tmp/rgb_%d.jpg' % i, cv2.cvtColor(np.array(canvas_rgb), cv2.COLOR_BGR2RGB))
