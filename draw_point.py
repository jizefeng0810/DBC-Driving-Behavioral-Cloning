...
def train_or_eval(teacher, net, data, optim, is_train, config):
    ...
    for i, (rgb_forward, rgb, mapview_forward, mapview, _, _) in enumerate(iterator):
        ...
        teacher_input = torch.cat((mapview_forward, mapview), 1)

        waypoints_map, waypoints = teacher.predict(teacher_input)
