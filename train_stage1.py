...
def train_or_eval(net, data, optim, is_train, config):
    ...
    for i, (_, _, mapview_forward, mapview, waypoints, _) in enumerate(iterator):
        mapview_forward = mapview_forward.to(config['device'])
        mapview = mapview.to(config['device'])
        waypoints = waypoints.to(config['device'])

        model_input = torch.cat((mapview_forward, mapview), 1)
...
def main(config):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(   # 50% and 75% 修改学习率 lr * gamma
    ...
    if wandb.run.resumed:
        resume_project(net, optim, scheduler, config)   # load
    else:
    ...
    for epoch in tqdm.tqdm(range(resume_epoch+1, config['max_epoch']+1), desc='epoch', position=0):
        ...
        checkpoint_project(net, optim, scheduler, config)   # save
...
if __name__ == '__main__':
    ...
    run_name  = 'stage1' + '_'.join(str(getattr(parsed, x)) for x in keys)  # 'stage1resnet18_0.0001_5e-06_128_1.0'
